...
function FormulaSubjectTopics({ subject }) {
    ...
    const fetchPdfs = async () => {
      try {
        ...
        const filteredPdfs = data.filter(pdf => 
          pdf.type === "formula" && pdf.subject === subject
        );
      ...
      <div className="topics-container">
        {pdfs.length > 0 ? (
          pdfs.map((pdf) => (
            ...
            //  to={`/formulas/${subject}/pdf-viewer/${encodeURIComponent(pdf.pdfPath)}`}
              className="link"
            >
...
FormulaSubjectTopics.propTypes = {
  subject: PropTypes.string.isRequired
};

export default FormulaSubjectTopics;