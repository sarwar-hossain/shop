...
const MyDataPage = () => {
  ...
  const handleUpdateClick = (id) => {
    navigate(`/updatemydata/${id}`); // Navigate to the update page with the item ID
  };

  return (
    <div className="my-data-page">
      <h1>MyData Page</h1>
      <button className="upload-pdf-button" onClick={() => navigate("/add-mydata")}>
        Upload New Item
      </button>
            ...
            <li key={item._id} className="my-data-item">
              <div className="image-container">
                <img
                  ...
                  src={`http://localhost:3000${item.status}`}
                  alt="MyImage"
                  onError={(e) => {
